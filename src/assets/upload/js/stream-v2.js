...
(function(){
	...
	function fRemoveEventListener(a, b, c) {
		try {
			...
			//console && console.log(e.name + ": " + e.message);
		}
	}
	...
	function SWFProvider(a) {
		...
		this.config = {
			...
			retryCount : 3,
			postVarsPerFile : {},
			swfURL : "/assets/custom/upload/swf/FlashUploader.swf",
			uploadURL : "/fd"
		};
	...
	StreamProvider.prototype = {
		...
		bindDropArea : function() {
			...
			//changed by Zhengyy
			var bg = document.getElementById("i_stream_dropzone_bg");
			if(null !== a && bg){
				document.body.ondragenter = function (ev) {
					setTimeout(function () {
						document.getElementById("i_stream_dropzone_bg").style.display = "block";
					},0);
				}
				document.body.ondrop = function (ev) {
					ev.stopPropagation();
					ev.preventDefault();
					document.getElementById("i_stream_dropzone_bg").style.display = "none";
				}
	...
	function Main(cfg){
		...
		this.uploadInfo = {};
        this.deletedTokens = new Array();
		this.config = {
			enabled : cfg.enabled == null ? true : !!cfg.enabled,//!0,
			customered : cfg.customered == null ? true : !!cfg.customered,//!!cfg.customered,
			multipleFiles : cfg.multipleFiles == null ? true : !!cfg.multipleFiles,//!!cfg.multipleFiles,
			...
			dragAndDropTips: cfg.dragAndDropTips || "请把文件(文件夹)拖放到这个区域，就可作为附件添加。",
			...
			onSelect : cfg.onSelect || this.onSelect,
			onAddTask: cfg.onAddTask || this.onAddTask,
			onMaxSizeExceed : cfg.onMaxSizeExceed || this.onMaxSizeExceed,
			onFileCountExceed : cfg.onFileCountExceed || this.onFileCountExceed,
			onExtNameMismatch : cfg.onExtNameMismatch || this.onExtNameMismatch,
			onCancel : cfg.onCancel || this.onCancel,
			onStop : cfg.onStop || this.onStop,
			onCancelAll : cfg.onCancelAll || this.onCancelAll,
			onComplete : cfg.onComplete || this.onComplete,//
			onQueueComplete: cfg.onQueueComplete || this.onQueueComplete,
			onUploadProgress: cfg.onUploadProgress ||this.onUploadProgress,//
			onUploadError: cfg.onUploadError || this.onUploadError,
			onDestroy: cfg.onDestroy || this.onDestroy,
			...
			swfURL : cfg.swfURL || "/assets/custom/upload/swf/FlashUploader.swf",
			...
			downloadURL : cfg.downloadURL || "/download",
			initFiles : cfg.initFiles || [],
			varName : cfg.varName || "_stream",
			uploadContainerId : cfg.uploadContainerId || "i_upload_container",
			totalMsgId : cfg.totalMsgId || "i_stream_total_msg",
			errorMsgId : cfg.errorMsgId || "i_stream_error_msg"
		};
		Parent.apply(this, arguments);
	}
	...
	Main.prototype = {
		...
		initializer : function() {
			...
			//fAddEventListener(window, "beforeunload", fExtend(this.unloadHandler, this));//IE10 上传多个文件会提示，暂时去掉
			this.waiting = [];
			this.uploading = !1;
			this.downloading = !1;
			this.totalFileSize = this.loadFiles(this.config.initFiles); //changed by Zhengyy ; this.totalFileSize = 0
			this.totalUploadedSize = this.totalFileSize; //changed by Zhengyy  this.totalUploadedSize = 0
			//this.showBrowseBlock();//changed by Zhengyy
			if (!this.config.customered) {this.showBrowseBlock();}
			//<iframe id="downloadframe" style="display:none"></iframe>
			var ifr = document.createElement("iframe");
			ifr.id="downloadframe";
			ifr.style.display="none";
			document.body.appendChild(ifr);
			var bg = document.getElementById("i_stream_dropzone_bg");
			var selectFiles = document.getElementById(this.config.browseFileId);
			if(this.config.dragAndDropArea != null && bg && selectFiles) {
				var top = selectFiles.offsetTop;
				var height = selectFiles.offsetHeight;
				var a = document.getElementById(this.config.dragAndDropArea);
				if(a) a.style.top = (top - height) + "px";
			}
			//end
		},
		...
		loadFiles: function (files) {
			var total = 0;
			//var streamInstance = eval(this.config.varName);
			for (var i = 0; i < files.length; i++) {
				var file = files[i];
				total += parseInt(file.size);
				var icon_str = this.getFaIcon(file.name);
				var size = this.formatBytes(file.size);
				//console.log(i+"_size:"+file.size);
				var buttons_str_success = "";
				var txt_success = "";
				if(!this.config.enabled){
					if( typeof xjj_savefile === 'function' ){
						buttons_str_success =
							'<a href="javascript:' + this.config.varName + '.download(\'' + file.token + '\',\'' + file.name + '\')" class="pure-link">下载</a>|'+
							'<a href="javascript:var option = new Object;option.source = \'\';var array = new Array();var arrayObj = new Object;arrayObj.id=\'' + file.token + '\';arrayObj.name=\'\';array.push(arrayObj);option.array = array;xjj_savefile(option);" class="pure-link">收藏</a>';
					}else{
						buttons_str_success =
							'<a href="javascript:' + this.config.varName + '.download(\'' + file.token + '\',\'' + file.name + '\')" class="pure-link">下载</a>';
					}
					...
					if( typeof xjj_savefile != 'function' ){
						buttons_str_success =
							'<a href="javascript:' + this.config.varName + '.download(\'' + file.token + '\',\'' + file.name + '\')" class="pure-link">下载</a>|' +
							'<a href="javascript:' + this.config.varName + '.removeFile(\'' + file.token + '\', \'' + file.token + '\', \''+file.name+'\', ' + file.size + ')" class="pure-link">删除</a>';
					}else{
						buttons_str_success =
							'<a href="javascript:var option = new Object;option.source = \'\';var array = new Array();var arrayObj = new Object;arrayObj.id=\'' + file.token + '\';arrayObj.name=\'\';array.push(arrayObj);option.array = array;xjj_savefile(option);" class="pure-link">收藏</a>|' +
							'<a href="javascript:' + this.config.varName + '.removeFile(\'' + file.token + '\', \'' + file.token + '\', \''+file.name+'\', ' + file.size + ')" class="pure-link">删除</a>';
				...
				var _file = '<div class="pure-u-1-3  G0" id="' + file.token + '">' +
					'  <i class="fa ' + icon_str + '  fa-2x"></i>' +
					'  <div class="o0" title="' + file.name + '">' + file.name + '</div>' +
					'  <div class="stream-buttons">' + buttons_str_success + '</div>' +
					'  <div class="j0">' +
					'     <div class="stream-process">' +
					'       <span class="stream-progress-text">' + size + '<span class="pure-txt-success">'+txt_success+'</span></span>' +
					'    </div>' +
					'  </div>' +
					'  <div class="stream-process-tip" style="display: none;">' + file.name + '<span></span></div>' +
					'</div>';
				$("#" + this.config.uploadContainerId).append(_file);
			}
			var $total = $("#"+this.config.totalMsgId);
			var totalFormat = this.formatBytes(total);
			//console.log("total:"+total);
			//$total.text("(100%, 已上传" + totalFormat + ", 总文件大小" + totalFormat + ")");
			return total;
		},
		...
		completeUpload : function(info) {
			...
			this.get("onComplete") ? this.get("onComplete")(info,this.config) : this.onComplete(info,this.config);
		...
		onMaxSizeExceed : function(file) {
			...
			alert("文件[" + file.name + "," + file.formatSize + "]大小超过限制(" + file.formatLimitSize + "),无法上传");
		},
		onExtNameMismatch: function(file) {
			fShowMessage("Allow ext name: [" + file.filters.toString() + "], not for " + file.name, true);
			alert(file.name + "的文件类型超出允许范围[" + file.filters.toString() + "],无法上传");
		},
		onAddTask: function(file,instConfig) {
			fShowMessage("Add to task << name: [" + file.name);
			var streamInst = eval(instConfig.varName);
			var icon_str = streamInst.getFaIcon(file.name);
			//var icon_str = this.getFaIcon(file.name);
			var buttons_str_process = '<a href="javascript:'+instConfig.varName+'.cancelOne(\'' + file.id + '\')" class="pure-link">删除</a>';
			var _file =
				'<div class="pure-u-1-3  G0" id="' + file.id + '">' +
				'  <i class="fa ' + icon_str + ' fa-2x"></i>' +
				'  <div class="o0" title="' + file.name + '">' + file.name + '</div>' +
			...
			$("#"+instConfig.uploadContainerId).append(_file);
			$("#" + file.id).hover(
				function () {
		...
		onUploadProgress: function (file, instConfig) {
...
//            $bar.css("width", file.percent + "%");
			...
			$bar_text.html(file.percent + '%' + ',\u5269\u4f59\u65f6\u95f4' + file.formatTimeLeft);
			$bar_tip_span.html(", 文件大小:" + file.formatSize + ", 已上传" + file.formatLoaded + ", 上传速度:" + file.formatSpeed);
//            信息化项目管理系统-正本-zyy0604.docx, 文件大小:11.86M, 已上传2.75M, 上传速度:311.30K/S
			var $total = $("#"+instConfig.totalMsgId);
//            (36%,已上传4.27M,总文件大小11.86M)
		...
		onCancel : function(file, instConfig) {
		...
		onComplete : function(file, instConfig) {
			...
			$file.find("div .stream-process-bar").hide('fast', function () {
				//858.32K <span class="pure-txt-success">上传完成</span>
				if(msg.message == ""){
                    $bar_text.html(file.formatLoaded + ' <span class="pure-txt-success">上传完成</span>');
                }else{
                    $bar_text.html(file.formatLoaded + ' <span class="pure-txt-warn" title="'+msg.message+'">上传完成 '+msg.message+' </span>');
                }

			});

			var $buttons = $("#" + file.id).find(".stream-buttons");
//            console && console.log("$buttons:" + $buttons.html());
			/*var buttons_str_success =
				'<a href="javascript:'+instConfig.varName+'.download(\'' + file.token + '\',\'' + file.name + '\')" class="pure-link">下载</a>|' +
				'<a href="javascript:'+instConfig.varName+'.removeFile(\'' + file.id + '\',\'' + file.token + '\')" class="pure-link">删除</a>';*/
			var buttons_str_success = '';
			if( typeof xjj_savefile != 'function' ){
				buttons_str_success =
					'<a href="javascript:'+instConfig.varName+'.download(\'' + file.token + '\',\'' + file.name + '\')" class="pure-link">下载</a>|' +
...
//            (36%,已上传4.27M,总文件大小11.86M)
		...
		onUploadError : function(status, msg, instConfig) {
			fShowMessage("Error Occur.  Status:" + status + ", Message: " + msg, true);
			var ob = document.getElementById(instConfig.errorMsgId)
			if(ob) document.getElementById(instConfig.errorMsgId).style.display = "block";
			var msgOb = JSON.parse(msg);
			alert(msgOb.name + ', '+msgOb.message);
		},
		disable : function() {
		...
		cancelOne : function(file_id, stopping, fileName, fileSize) {
			var _UploadInfo = this.uploadInfo[file_id] || false;
			var provider = _UploadInfo ? this.uploadInfo[file_id].file : null, actived = _UploadInfo ? _UploadInfo.actived: false;
			provider && provider.cancelUpload && provider.cancelUpload();
			if (!!stopping) return true;

			//change by zhengyy
			var upFileSize = _UploadInfo ? _UploadInfo.file.config.size : fileSize;
			var totalSize = this.totalFileSize - upFileSize;
			if(!_UploadInfo ||  _UploadInfo.uploadComplete){
				this.totalUploadedSize = this.totalUploadedSize  - upFileSize;
			}
			//end
			var info = {
				id:   file_id,
				name:  _UploadInfo ? _UploadInfo.file.config.name : fileName,
				size: upFileSize, //this.uploadInfo[file_id].file.config.size,
				totalSize: totalSize,
			...
			this.get("onCancel") ? this.get("onCancel")(info,this.config) : this.onCancel(info,this.config);
			...
			var size = provider ? provider.config.size : fileSize;
		...
		uploadProgress : function(a) {
			...
			if (this.config.customered) {
				...
				this.get("onUploadProgress") && this.get("onUploadProgress")(info,this.config);
				return false;
			}
		...
		uploadComplete : function(a) {
			...
			var respJson = null;
			var token = (respJson = eval("(" + a.data + ")")) ? respJson.token : "";
			...
			if (!this.uploadInfo[id]) return false;
			this.uploadInfo[id].uploadToken=token; //add by zhengyy
			var info = {
				id:                 id,
				token:             token, //add by zhengyy
		...
		uploadError : function(evt) {
			this.get("onUploadError") ? this.get("onUploadError")(evt.status, evt.statusText, this.config) : this.onUploadError(evt.status, evt.statusText, this.config);
		},
		...
		validateFile : function(uploader) {
			...
			valid && this.config.customered && this.get("onAddTask")(info,this.config) && this.get("onAddTask")(info,this.config);
			return valid;
		},
		...
		removeFile : function(id,token,fileName,fileSize) {
			this.cancelOne(id,null,fileName,fileSize);
			this.deletedTokens.push(token);
			//console && console.log("remove file. token:"+token);
		},
		retry : function() {
			this.stop();
			this.upload();
			var ob = document.getElementById("i_stream_error_msg")
			if(ob) document.getElementById("i_stream_error_msg").style.display = "none";
		},
		download : function(token,showName) {
			var vars = { token : token, showName : showName};
			var downloadURL = fAddVars(vars, this.get("downloadURL")) + "&" + fGetRandom();
			if(!this.downloading){
				this.downloading=!0;
				var myframe=document.getElementById('downloadframe');
				myframe.src=downloadURL;
				this.downloading=!1;
			}
		},
		getTokens :function(){
			var tokens = new Array;
			for (var c in this.uploadInfo){
				if(this.uploadInfo[c].uploadComplete){
					tokens.push(this.uploadInfo[c].uploadToken)
				}
			}
			return tokens;
		},
		getDeletedTokens :function(){
		...
		getFaIcon : function(fileName) {
			var suffix = fileName.substr(fileName.lastIndexOf("."));
	//        console && console.log(fileName +"  suffix;"+suffix);
			var reg_image = /^.(JPEG|JPG|TIFF|RAW|BMP|GIF|PNG|TIFF|PSD|SWFSVG|EMF|ICO|PCX|DXF|WMF|LIC|EPS|TGA)$/gi;
			var reg_excel = /^.(XLS|XLSX|CSV|ET|ETT|XLT)$/gi;
			var reg_word = /^.(DOC|DOCX|WPS|WPT|DOT|RTF)$/gi;
			var reg_pdf = /^.(CEB|PDF)$/gi;
			var reg_powerpoint = /^.(DPS|DPT|PPT|PPTX|POT|PPS)$/gi;
			var reg_zip = /^.(RAR|ZIP|EXE|ARJ|CAB|LZH|ACE|TAR|GZ|UUE|BZ2|JAR|ISO|7Z)$/gi;
			var reg_audio = /^.(ACT|REC|VY1|VY2|VY3|VY4|SC4|DVF|MSC|WMA|MP3|WAV|MP3|AMR|AVA)$/gi;
			var reg_video = /^.(WMV|ASF|ASX|MOV|RM|RMVB|MPG|MPEG|MPE|3GP|MP4|M4V|AVI|DAT|MKV|FLV|VOB)$/gi;
			var reg_text = /^.(TXT|LOG|XML|PROPERTIES|YAML|INI|CFG)$/gi;
			var reg_code = /^.(H|CPP|DLL|LIB|DSP|ASP|ASPX|JSP|PHP|HTM|HTML|CSS|JS|VB|JAR|BAT|SH|PY)$/gi;
			if (!suffix || "") return "fa-file-o";
			if (reg_image.test(suffix)) return "fa-file-image-o";
			if (reg_word.test(suffix)) return "fa-file-word-o";
			if (reg_excel.test(suffix)) return "fa-file-excel-o";
			if (reg_pdf.test(suffix)) return "fa-file-pdf-o";
			if (reg_powerpoint.test(suffix)) return "fa-file-powerpoint-o";
			if (reg_zip.test(suffix)) return "fa-file-zip-o";
			if (reg_text.test(suffix)) return "fa-file-text-o";
			if (reg_code.test(suffix)) return "fa-file-code-o";
			if (reg_audio.test(suffix)) return "fa-file-audio-o";
			if (reg_video.test(suffix)) return "fa-file-video-o";
			return "fa-file-o";
		}
